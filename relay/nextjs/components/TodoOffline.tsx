  ...
  return (
    <div className="mainrequest">
      ...
      <ul className="listmutation">
        {!isMultiple && !todoBackup.__UNPUBLISH_RECORD_SENTINEL ? (
          <Todo disabled todo={todoBackup} user={user} relay={{}} />
        ) : (
          <div className="emptymutation" />
        )}

        {!isMultiple && todoPublish ? (
          <Todo disabled todo={todoPublish} user={user} relay={{}} />
        ) : (
          <div className="emptymutation" />
        )}
      </ul>
...
const TodoOffline = ({relay, user}) => {
  const offlineState = useOffline();

  const requests: Array<OfflineRecordCache<Payload>> = [];
